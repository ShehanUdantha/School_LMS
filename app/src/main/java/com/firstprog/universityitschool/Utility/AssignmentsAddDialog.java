...
public class AssignmentsAddDialog {

    private Context context;
    private Dialog assignmentAddDialog;
    ...
    public AssignmentsAddDialog(Context context) {
        this.context = context;
    }

    //display editText view when clicking add button
    public void showDialog() {
        assignmentAddDialog = new Dialog(context);
        assignmentAddDialog.setContentView(R.layout.pdf_and_video_add_dialog);
        assignmentAddDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        assignmentAddDialog.setCanceledOnTouchOutside(true);

        uploadCardName = assignmentAddDialog.findViewById(R.id.uploadCardName);
        uploadCardName.setText("Add New Assignment");
        pdfScreenTitle = assignmentAddDialog.findViewById(R.id.uploadScreenTitle);
        pdfScreenTitle.setText("Tap to Upload Assignment");

        assignmentAddDialog.create();
        assignmentAddDialog.show();
    }


    //return button id to week pdf Fragment
    public Button submit() {
        return submit = assignmentAddDialog.findViewById(R.id.btnSubmitFile);
    }

    public LinearLayout uploadAreaView(){
        return uploadArea_ = assignmentAddDialog.findViewById(R.id.uploadArea);
    }
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot ds : snapshot.getChildren()) {
                            ...
                            if (allSemID.equals(weekID)) {
                                newDatabaseReference = ds.child("week_assignments").getRef();
                                // Generate a reference to a new location and add some data using push()
                                pushedPostRef = newDatabaseReference.push();
                                // Get the unique ID generated by a push()
                                String postId = pushedPostRef.getKey();

                                newDatabaseReference.child(postId).child("assignmentsMainChildID").setValue(postId);
                                newDatabaseReference.child(postId).child("assignmentName").setValue(fileName);
                                String date = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date());
                                newDatabaseReference.child(postId).child("assignmentUploadDate").setValue(date);
    ...
    public void getPDFUrl(String weekID, String weekPreKey, String postID, Uri fillPath) {
        ...
        ref = storageReference.child("materials").child(weekID).child("assignment/" + UUID.randomUUID().toString() + "." + "pdf");
                    ...
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        ...
                        taskSnapshot.getStorage().getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                            ...
                            public void onComplete(@NonNull Task<Uri> task) {
                                databaseReference.child(weekPreKey).child("week_assignments").child(postID).child("assignmentUrl").setValue(task.getResult().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        assignmentAddDialog.dismiss();
